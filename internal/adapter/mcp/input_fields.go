...
func v2ActionFields(action string) []string {
	switch action {
	case "start":
		return []string{"operation_id", "workspace_id", "activity_id", "workspace_hint", "structured_adapter", "project_command_id", "params", "command", "argv", "intent", "evidence", "cwd", "tty", "persistent", "session_name", "yield_time_ms", "timeout_ms", "trace_mode", "limits", "hermetic", "max_output_bytes"}
	case "poll":
