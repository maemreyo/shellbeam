...
func (p *Provider) queryFacts(ctx context.Context, c *controlClient, target string) (tmuxFacts, error) {
	line, err := tmuxCommandLine("display-message", "-p", "-t", target, "#{session_id}|#{window_id}|#{pane_id}|#{pane_pid}|#{q:pane_current_command}|#{q:pane_tty}|#{q:pane_current_path}|#{pane_dead}|#{pane_dead_status}|#{q:socket_path}|#{pid}|#{q:version}")
	if err != nil {
	...
	return parseTmuxFacts(result.Data)
}

func parseTmuxFacts(data string) (tmuxFacts, error) {
	parts, err := splitEscapedTmuxFacts(data)
	...
	serverPID, err := strconv.Atoi(parts[10])
	...
	var code *int
	if terminal && parts[8] != "" {
		v, err := strconv.Atoi(parts[8])
		if err != nil {
			return tmuxFacts{}, fmt.Errorf("invalid pane exit status")
		}
		code = &v
	}
	paneTTY := contextExecPaneTTY(parts[5])
	cwd := contextExecCWD(parts[6])
	return tmuxFacts{
		SessionInternalID: parts[0], WindowID: parts[1], PaneID: parts[2],
		PanePID: panePID, ServerPID: serverPID, Terminal: terminal, ExitCode: code,
		CurrentCommand: currentCommand, PaneTTY: paneTTY, CWD: cwd,
		SocketPath: parts[9], Version: "tmux " + strings.TrimPrefix(parts[11], "tmux "),
	}, nil
}

func splitEscapedTmuxFacts(data string) ([]string, error) {
	...
	for i := 0; i < len(data); i++ {
		...
		case '|':
			...
			field.Reset()
		case '\x00', '\r', '\n':
			return nil, fmt.Errorf("invalid tmux facts")
		default:
			field.WriteByte(c)
		}
	}
	if escaped {
		return nil, fmt.Errorf("invalid tmux facts escape")
	}
	parts = append(parts, field.String())
	return parts, nil
}

func contextExecPaneTTY(value string) string {
...
func contextExecCWD(value string) string {
	if value == "" || strings.ContainsAny(value, "\x00\r\n") || !filepath.IsAbs(value) {
		return ""
	}
...
func (p *Provider) observationFromFacts(c *controlClient, state privateState, facts tmuxFacts) app.Observation {
	...
	return app.Observation{Provider: p.Identity(), ProviderCurrent: true, ProviderGeneration: state.ProviderGeneration, Owner: owner, Terminal: facts.Terminal, ExitCode: facts.ExitCode, PanePID: facts.PanePID, CurrentCommand: facts.CurrentCommand, PaneTTY: facts.PaneTTY, CWD: facts.CWD, OutputBytes: c.outputByteCount()}
}
