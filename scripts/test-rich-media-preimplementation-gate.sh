...
for p,s in [('plan.md','plan\n'),('spec.md','spec\n'),('source.md','source\n'),('preflight.md','verdict = PASS\n'),('phase.md','phase\n'),('parser.md','parser\n'),('tracer.json','{\"verdict\":\"PASS\",\"exit_status\":0,\"candidate_mode\":\"go1.26-pinned-json-library-boundary\",\"goexperiment\":\"\",\"module_version\":\"v0.0.0-20260623181947-01eb4420fa68\"}\n'),('macos.json','{\"status\":\"PASS\"}\n'),('linux.json','{\"status\":\"PASS\"}\n')]: write(p,s)
...
def manifest(): return {
 ...
 'phase_a':{'topology':'one-tool','host_offers_remembered_approval':False,'checks':[row(i) for i in phase_ids]+[
   ...
   'phase_a_pass':True},
 'parser_toolchain':{'candidate':'go1.26-pinned-json-library-boundary','go127_ga':False,'explicit_go126_experiment_acceptance':False,'mode':'library-boundary','go_version':'go1.26.5','goexperiment':'','module_version':'v0.0.0-20260623181947-01eb4420fa68','strict_tracer_report':{'path':'tracer.json','sha256':sha('tracer.json')},'native_lanes':{'macos':{'status':'PASS','mode':'library-boundary','go_version':'go1.26.5','goexperiment':'','module_version':'v0.0.0-20260623181947-01eb4420fa68','evidence':{'path':'macos.json','sha256':sha('macos.json')}},'linux':{'status':'PASS','mode':'library-boundary','go_version':'go1.26.5','goexperiment':'','module_version':'v0.0.0-20260623181947-01eb4420fa68','evidence':{'path':'linux.json','sha256':sha('linux.json')}}},'checks':[{'id':i,'status':'PASS','provenance':['synthetic-valid-fixture']} for i in parser_ids],'parser_toolchain_pass':True},
...
m=manifest();m['parser_toolchain']['module_version']='v0.0.0-deadbeef';expect('mismatched-module',m,4)
