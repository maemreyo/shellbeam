...
- Modify: `internal/adapter/store/reconcile_test.go`
...
- Generic daemon crash reconciliation (`AbandonUnresolved`) must not preempt delegated recovery. A schema-5 delegated reservation with a valid delegated recovery marker is deferred to the delegated startup coordinator (including interrupted marker→ref→binding creation that the marker can repair). A live/provisioning binding with a missing marker is state corruption and blocks startup; only a delegated reservation that never reached any recovery marker/binding may be canonicalized by the generic path, using a valid schema-5 ambiguous receipt with `transport_gap` rather than a legacy-shaped receipt.
...
- `transport_gap` is capture truth, not by itself child-process failure. When the qualified provider proves terminal exit 0 after restart, receipt schema 5 preserves `state=completed + outcome=success` while setting `output_complete=false + capture_quality=incomplete + capture_reasons=[transport_gap]`. Schemas 1–4 keep their legacy success-requires-complete-output rule. A real observer/durable-byte mismatch remains `output_capture_failed`.
