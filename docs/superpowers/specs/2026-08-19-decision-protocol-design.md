...
Date: 2026-08-19
Status: semantic freeze approved; implementation planning may proceed after independent review
...
Activation authority follows the existing ShellBeam policy-activation authority domain until separately extended. Decision Protocol override authority is a different domain and MUST NOT reuse `DerivationAuthority`.

### 6.3 Episode policy immutability

Each episode binds exact:
...
The evaluator considers the **complete compatible qualified verification set** at the frozen observation cut. It MUST NOT select one favorable `EvidenceCandidate` while ignoring contradictory qualifying evidence.
...
`declared_provider_identity` is caller-declared provenance unless independently qualified by a future provider contract.
...
An assessment supports candidate B only if it is valid for the episode, meets exact required provenance qualification, and B is in `preferred_candidates`.

No confidence threshold, majority voting, or aggregate score exists in V1.

Unknown/unqualified required provenance yields `INDETERMINATE`, never silent downgrade.
...
It excludes literal operation/evidence IDs when replacing them with equivalent facts leaves epistemic state unchanged.

### 30.2 Audit digest

`DecisionAuditDigest` includes exact historical identity and references.
...
Authority qualification is a provider/resolver boundary invoked by the selection-commit path. It is not a generic model instruction to self-assert a role.
...
An episode exists only when a caller explicitly creates one and binds an activated policy.
...
3. **Experiment execution admission:** at most one observation-producing operation link may be created in V1.
4. **Experiment terminal records:** close and abort are mutually exclusive.
5. **Observation materialization:** a canonical binding represents one complete derivation cut and all sealed predictions.
6. **Episode terminalization:** SelectionCommit and DecisionClosure are mutually exclusive and serialized.
7. **Normal vs override selection:** same candidate does not collapse distinct epistemic commit semantics.
8. **Projection CAS:** commit is authorized only against the exact current semantic projection requested by the caller.
9. **Override authorization:** qualification and local terminal commit use a clearly defined local authorization point; failed persistence requires requalification on retry.

## 55. Security and authority invariants

1. Caller-declared identity/role is not qualified authority by itself.
2. V1 performs exact authority-class matching only.
3. Unknown/unavailable resolver result fails closed.
4. An attestation proves capability; an override proves bounded intent.
5. Override intent is stale when blocker digest changes.
6. Override never bypasses source/structural/CAS integrity.
7. Historical successful authorization is not reinterpreted under current resolver state.
8. Durable idempotent replay does not re-authorize already committed history.
9. V1 core does not claim ownership of external provider revocation state.
...
2. Episode policy snapshot/activation is durably bound and immutable for episode lifetime.
3. Candidate revision is immutable replacement with active-parent CAS.
4. Candidate revision does not implicitly inherit predictions.
5. Prediction declaration happens-before experiment observability.
6. Experiment definition/seal/close never schedules execution.
7. V1 permits at most one observation-producing execution link per experiment.
8. Execution joins an experiment only through admission-time binding.
9. Protocol observation truth is server-derived, complete for the linked observation domain, and non-cherry-pickable.
10. Every linked execution must have a terminal observation binding before selection commit, including post-link aborted experiments.
11. ShellBeam evaluates typed declared expectations, never semantic hypothesis truth.
12. V1 predicate kinds are closed and versioned; there is no generic expression DSL.
13. REQUIRED mismatch is an implicit candidate-contract blocker only when determinately evaluated.
14. Unevaluated or indeterminate REQUIRED prediction does not silently become a blocker.
15. Candidate-scoped policy evaluation uses four bounded V1 requirement kinds: challenge, prediction evaluation, discrimination, verifier assessment.
16. Discrimination qualification is frozen at seal time against then-eligible candidate lineages.
17. Realized discrimination is experiment-local against a replayable seal-time cut and excludes its own aggregate requirement/gate status.
18. Verification predicates evaluate the complete compatible qualified evidence set, not a favorable individual record.
19. Protocol fold is deterministic: BLOCKED > INDETERMINATE > CLEAR.
20. Budget admission is a separate ceiling and never changes protocol satisfaction.
21. Projection digest represents semantic epistemic/protocol state; audit digest represents exact history.
22. `expected_projection_digest` is the semantic CAS token for selection commit.
23. Source-generation mismatch fail-closes V1 selection and cannot be overridden.
24. Selection proposal has no transition authority.
25. Selection commit and close-unresolved share one atomic episode-terminal boundary.
26. Normal and override commits are epistemically distinct even for the same candidate.
27. Override never rewrites blockers as satisfied.
28. DecisionAuthorityAttestation is immutable capability proof; current qualification is resolver-derived.
29. V1 has no authority lattice, wildcard authority equivalence, or generic core revocation ledger.
30. Override authority is revalidated at terminal commit authorization time.
31. Only `QUALIFIED` exact-class/exact-scope authority may authorize a new override commit.
32. Override intent covers the exact current candidate/policy/projection/blocker set.
33. SelectionCommit persists the exact override authorization cut used to authorize history.
34. Later expiry/revocation does not rewrite a durable historical commit.
35. Failed/non-durable commit retry must revalidate authority; replay of durable idempotent success does not.
36. `VerifierAssessment` is a separate non-evidence ontology.
37. Policy/projection exposes constraints and allowed protocol transitions, never recommended engineering next actions.
38. Candidate committed does not imply implementation verified or task done.
39. Oracle-gap metrics are unavailable unless candidate evaluation is genuinely exhaustive under a qualifying oracle.
40. `CLOSED_UNRESOLVED` remains the truthful terminal escape from an unsatisfied or unresolvable protocol.
