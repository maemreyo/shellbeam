...
The 8 MiB image limit is a **candidate release ceiling** until the real tunnel/ChatGPT and memory gates pass.

For 8 MiB raw bytes:

```text
base64.StdEncoding.EncodedLen(8 MiB) = 11,184,812 bytes
outer media response ceiling = 11,184,812 + 65,536
                            = 11,250,348 bytes
```
...
require size <= 8 MiB
read at most 8 MiB + 1 from finalFD
fstat(finalFD) again
...
For the candidate 8 MiB limit:

```text
maxOuter = 11,250,348 bytes
```
...
If the 8 MiB path fails the Section 21 scorecard, lower the explicit candidate product limit before implementation planning and rerun the affected matrix. Do not silently ship a lower effective ceiling.
...
- idle baseline;
- one 8 MiB read peak;
- two concurrent 8 MiB read peak;
...
The original 8 MiB raw image ceiling remains provisional until real ChatGPT transport and memory gates pass. The original 5 s wording remains a cooperative acquisition budget, not a hard syscall interruption guarantee.
