...
- Local disclosure/correlation qualification now also proves the tool description preserves original-file-byte + embedded-metadata + client/model-egress facts; actual MCP results preserve disclosure text before `ImageContent`, exact `action=read_media`, complete `display_address`, and wrong-base rejection. These are **not** substitutes for Section 21.6's host pre-result UI/confirmation timing gate, which remains unrun.
- OpenAI's current Apps documentation (`https://help.openai.com/en/articles/11487775-connectors-in-chatgpt`, checked 2026-08-17) documents persistent app-permission options such as `Never ask` and permission cards that may expose `Always allow`, while noting availability varies by app/action/account. Therefore remembered approval cannot be declared `not_applicable` from product-class assumptions; availability for this exact Phase A app/action remains unestablished and the row stays `NOT_RUN`.
...
| disclosure-confirmation | 1 / 1 | 0 | 0 | 1 | NOT_RUN | local disclosure/result correlation PASS; host pre-result UI/confirmation timing not recorded |
| production-disclosure | 1 / 1 | 0 | 0 | 1 | NOT_RUN | test-tool disclosure semantics PASS; production host disclosure timing not run |
